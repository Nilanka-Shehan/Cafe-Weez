...
const ForgotPasswordPage = () => {
	...
	const { isLoading, forgotPassword,user,login} = useAuthStore();
	...
	const handleSubmit = async (e) => {
		...
		setIsSubmitted(true);
	// Redirect user after reset (if needed, based on their role or after a successful reset)
		if (user) {
			// If the user is already logged in or after they reset their password, navigate based on role
			if (user.role === "admin") {
				navigate("/dashboard"); // Admin page
			} else if (user.role === "kitchenStaff") {
				navigate("/kitchenstaff"); // Kitchen staff page
			} else if (user.role === "waiter") {
				navigate("/waiter"); // Waiter page
			} else {
				navigate("/home"); // Regular user page
			}
		} else {
			// If no user is available (or if forgotPassword does not log them in), inform them about the reset link
			// You can also trigger a login flow here if you want the user to be logged in automatically after reset
